...
var parameters ={
	...
	bird_view : function(){ bird_view();},
	camera_view : function(){ camera_view();}
...
function data_load(){
	...
	image_2d = new Image();
	image_2d.src = input_filename + '/JPEGImages/' + ( '000000'  + parameters.flame ).slice( -6 )  + '.jpg?' + new Date().getTime();
				...
				for (var i = 0 ; i < allText.split("\n").length ; i++){
					if(allText.split("\n")[i].split(",").length == 15){
						...
						var readfile_parameters ={
							...
							delta_y : 0,
							delta_z : 0,
							width : parseFloat(str.split(",")[8]),
							height : parseFloat(str.split(",")[9]),
							depth : parseFloat(str.split(",")[10]),
							yaw : parseFloat(str.split(",")[14]),
							numbertag : parameters.i + 1,
							label : str.split(",")[0]
						};
...
function addbbox_gui(num){
	...
	cube_delta_X.onChange(function(value){cube_array[num].position.x = each_cube_parameters[num].x + value;});
	cube_delta_Y.onChange(function(value){cube_array[num].position.y = -each_cube_parameters[num].y - value;});
	cube_delta_Z.onChange(function(value){cube_array[num].position.z = each_cube_parameters[num].z + value;});
...
function init() {
	...
	readYAMLFile("input/calibration.yml");
	data_load(parameters);
	readTextFile(input_filename + '/Annotations/' + ( '000000'  + parameters.flame ).slice( -6 ) + '.txt');
	gui.open();
	gui_add_tag();
	ImageCheck.onChange(function(value){if(bird_view_flag==false){image_array[0].visible = value;} else if(bird_view_flag==true && value==true) {
	canvas2D.style.display="block"} else if(bird_view_flag==true && value==false) {canvas2D.style.display="none"}})
